...
(function () {
  ...
  const openDonationEmail = ({ donorName, donorEmail, amountUsd, paymentMethod, campaign, givingFrequency, donorNote, anonymousDonation }) => {
    const subject = encodeURIComponent(`One World Relief donation - ${campaign}`);
    const body = encodeURIComponent(
      [
        ...
        `Campaign: ${campaign}`,
        `Schedule: ${givingFrequency}`,
        `Preferred payment method: ${paymentMethod}`,
        `Anonymous in public updates: ${anonymousDonation ? "Yes" : "No"}`,
        "",
        donorNote ? `Note: ${donorNote}` : "Note: none",
        "",
        "Please send me the best payment link to complete this donation.",
      ].join("\n")
    );
    window.location.href = `mailto:${CONTACT_EMAIL}?subject=${subject}&body=${body}`;
  };

  const sendCloudflareDonationRequest = async (payload) => {
    const response = await fetch(CLOUDFLARE_DONATION_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        schedule: payload.givingFrequency,
        payment_method: payload.paymentMethod,
        note: payload.donorNote,
        anonymous: payload.anonymousDonation,
      }),
    });
  ...
  donationForm.addEventListener("submit", async (event) => {
    ...
    if (!HAS_CHECKOUT_API) {
      ...
      donateButton.textContent = "Sending to Cloudflare...";
      try {
        await sendCloudflareDonationRequest(donationPayload);
        setStatus("Donation request sent through Cloudflare. Check your email for the next payment step.", false);
      } catch (_error) {
        openDonationEmail(donationPayload);
        setStatus("Cloudflare is not receiving requests on this preview yet, so a prefilled email opened instead.", false);
      } finally {
